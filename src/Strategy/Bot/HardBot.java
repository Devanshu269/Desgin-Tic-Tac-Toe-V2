...
public class HardBot implements BotPlayingStrategy {
    ...
    public Move makeMove(Board board, Player player) {
        ...
        int center = n / 2;
        if (n % 2 == 1 && cells[center][center].getCellStatus() == enums.CellStatus.EMPTY) {
            return new Move(player, center, center);
        }

        List<int[]> corners = new ArrayList<>();
        int[][] cornerCoords = new int[][]{{0,0},{0,n-1},{n-1,0},{n-1,n-1}};
        for (int[] cc : cornerCoords) {
            if (cells[cc[0]][cc[1]].getCellStatus() == enums.CellStatus.EMPTY) corners.add(cc);
        }
        if (!corners.isEmpty()) {
            Random rnd = new Random();
            int[] choice = corners.get(rnd.nextInt(corners.size()));
